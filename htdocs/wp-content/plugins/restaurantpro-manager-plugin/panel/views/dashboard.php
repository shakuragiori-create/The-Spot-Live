...
$orders_today = (int) $wpdb->get_var( $wpdb->prepare(
    "SELECT COUNT(*) FROM {$wpdb->prefix}rp_orders WHERE DATE(created_at) = %s", $today
) );
$revenue_today = (float) $wpdb->get_var( $wpdb->prepare(
    "SELECT COALESCE(SUM(total), 0) FROM {$wpdb->prefix}rp_orders WHERE DATE(created_at) = %s AND status <> 'cancelled' AND (paid_at IS NOT NULL OR amount_paid >= total)", $today
) );
