...
<a href="<?php echo esc_url(home_url('/panel/orders/'.(int)$o->id)); ?>" class="order-item rp-pay-order <?php echo $paid?'is-paid':'is-unpaid'; ?>" data-order-id="<?php echo (int)$o->id; ?>">
  ...
  <span style="display:flex;align-items:center;gap:8px"><span class="order-total">Rs.<?php echo number_format((float)$o->total,0); ?></span><span class="btn btn-sm btn-outline" data-bill="<?php echo esc_url(home_url('/panel/bill/'.(int)$o->id)); ?>"><?php echo rp_panel_icon('receipt'); ?></span></span>
</a>
