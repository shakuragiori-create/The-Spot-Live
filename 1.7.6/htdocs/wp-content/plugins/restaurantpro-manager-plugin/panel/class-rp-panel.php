...
class RP_Panel {
    ...
    private function get_allowed_pages( string $role ): array {
        $all_pages = [ 'dashboard', 'pos', 'orders', 'order-new', 'order-view', 'kitchen', 'tables', 'reservations', 'reports', 'accounts', 'menu', 'gallery', 'users', 'messages', 'settings', 'inventory', 'expenses', 'shifts', 'activity', 'backup' ];
        ...
        $saved = RP_Settings::get( 'role_pages_' . $role, '' );
        if ( $saved === '' ) {
            return $defaults;
        }

        $pages = json_decode( $saved, true );
        if ( ! is_array( $pages ) ) {
            return $defaults;
        }

        // Auto-include order-new and order-view when orders is allowed
        if ( in_array( 'orders', $pages, true ) ) {
            if ( ! in_array( 'order-new', $pages, true ) ) {
                $pages[] = 'order-new';
            }
            if ( ! in_array( 'order-view', $pages, true ) ) {
                $pages[] = 'order-view';
            }
        }
    ...
    public function get_nav_items( string $role ): array {
        ...
        // Build the full list of nav items
        $all_items = [
            [ 'url' => "$base/pos", 'icon' => 'dollar-sign', 'label' => 'POS', 'page' => 'pos' ],
            [ 'url' => $base, 'icon' => 'grid', 'label' => 'Dashboard', 'page' => 'dashboard' ],
            [ 'url' => "$base/orders", 'icon' => 'clipboard', 'label' => 'Orders', 'page' => 'orders' ],
            [ 'url' => "$base/messages", 'icon' => 'message', 'label' => 'Messages', 'page' => 'messages' ],
            [ 'url' => "$base/kitchen", 'icon' => 'flame', 'label' => 'Kitchen', 'page' => 'kitchen' ],
            [ 'url' => "$base/tables", 'icon' => 'layout', 'label' => 'Tables', 'page' => 'tables' ],
            [ 'url' => "$base/reservations", 'icon' => 'calendar', 'label' => 'Reservations', 'page' => 'reservations' ],
            [ 'url' => "$base/accounts", 'icon' => 'bar-chart', 'label' => 'Accounts', 'page' => 'accounts' ],
            [ 'url' => "$base/reports", 'icon' => 'grid', 'label' => 'Reports', 'page' => 'reports' ],
            [ 'url' => "$base/menu", 'icon' => 'book-open', 'label' => 'Menu', 'page' => 'menu' ],
            [ 'url' => "$base/gallery", 'icon' => 'image', 'label' => 'Gallery', 'page' => 'gallery' ],
            [ 'url' => "$base/users", 'icon' => 'users', 'label' => 'Staff', 'page' => 'users' ],
            [ 'url' => "$base/settings", 'icon' => 'settings', 'label' => 'Settings', 'page' => 'settings' ],
            [ 'url' => "$base/inventory", 'icon' => 'box', 'label' => 'Inventory', 'page' => 'inventory' ],
            [ 'url' => "$base/expenses", 'icon' => 'receipt', 'label' => 'Expenses', 'page' => 'expenses' ],
            [ 'url' => "$base/shifts", 'icon' => 'clock', 'label' => 'Cash Shift', 'page' => 'shifts' ],
            [ 'url' => "$base/activity", 'icon' => 'activity', 'label' => 'Activity Log', 'page' => 'activity' ],
            [ 'url' => "$base/backup", 'icon' => 'download', 'label' => 'Backup', 'page' => 'backup' ],
        ];

        $allowed = $this->get_allowed_pages( $role );

        foreach ( $all_items as $item ) {
            if ( in_array( $item['page'], $allowed, true ) ) {
                $items[] = $item;
            }
        }
    ...
    private function process_form( string $page, string $role ): void {
        ...
        if ( $page === 'settings' && isset( $_POST['rp_settings_save'] ) ) {
            ...
            // Save role access control settings (admin only)
            if ( $role === 'admin' && isset( $_POST['role_pages'] ) && is_array( $_POST['role_pages'] ) ) {
                $valid_pages = [ 'dashboard', 'pos', 'orders', 'order-new', 'order-view', 'kitchen', 'tables', 'reservations', 'reports', 'accounts', 'menu', 'gallery', 'users', 'messages', 'settings', 'inventory', 'expenses', 'shifts', 'activity', 'backup' ];
                $valid_roles = [ 'receptionist', 'kitchen', 'waiter' ];
                foreach ( $valid_roles as $r ) {
                    $pages = [];
                    if ( isset( $_POST['role_pages'][ $r ] ) && is_array( $_POST['role_pages'][ $r ] ) ) {
                        foreach ( $_POST['role_pages'][ $r ] as $p ) {
                            $p = sanitize_text_field( $p );
                            if ( in_array( $p, $valid_pages, true ) ) {
                                $pages[] = $p;
                            }
                        }
                        // Auto-include order-new and order-view when orders is selected
                        if ( in_array( 'orders', $pages, true ) ) {
                            if ( ! in_array( 'order-new', $pages, true ) ) {
                                $pages[] = 'order-new';
                            }
                    ...
                    RP_Settings::set( 'role_pages_' . $r, wp_json_encode( $pages ) );
                }
            }
